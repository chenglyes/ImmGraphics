...
void RenderObjects()
{
    ...
    phongShader->material.diffuse = Vec3(0.0f, 0.93f, 0.21f);
    phongShader->material.specular = Vec3(0.5f, 0.5f, 0.5f);
    phongShader->material.shininess = 32;
    renderer.Render(window->getDevice(), pipeline);
    renderer.Clear();
...
void RenderProc()
{
    ...
    BEGIN_TIMER(Normal Render 1)
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Normal Render 2) 
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Normal Render 3)
        RenderObjects();
    END_TIMER()
    ...
    BEGIN_TIMER(Render with Back-Culled 1)
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Render with Back-Culled 2)
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Render with Back-Culled 3)
        RenderObjects();
    END_TIMER()
    ...
    BEGIN_TIMER(Render with Fast-Sample and Back-Culled 1)
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Render with Fast-Sample and Back-Culled 2)
        RenderObjects();
    END_TIMER()

    window->getDevice()->ClearBuffer();

    BEGIN_TIMER(Render with Fast-Sample and Back-Culled 3)
        RenderObjects();
    END_TIMER()
